  ...
  public static $HMAC_SHA1_LEN = 20;
    ...
    $td = mcrypt_module_open(MCRYPT_RIJNDAEL_128, '', MCRYPT_MODE_CBC, '');
        if (is_callable('mb_substr')) {
            $iv = mb_substr($encrypted_text, 0, self::$CIPHER_BLOCK_SIZE, 'latin1');
        } else {
    ...
    if (is_callable('mb_substr')) {
        $encrypted = mb_substr($encrypted_text, self::$CIPHER_BLOCK_SIZE, mb_strlen($encrypted_text, 'latin1'), 'latin1');
    } else {
        $encrypted = substr($encrypted_text, self::$CIPHER_BLOCK_SIZE);
    }
