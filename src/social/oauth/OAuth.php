...
class OAuthRequest
{
  ...
  public static function from_request($http_method = null, $http_url = null, $parameters = null)
  {
    ...
    if ($parameters) {
      ...
      if (isset($request_headers['Authorization']) && @substr($request_headers['Authorization'], 0, 5) == 'OAuth') {
          ...
          $req = new self($http_method, $http_url, $req_parameters);
      }
    }
  ...
  {
      @$parameters or $parameters = [];
      $defaults = ['oauth_version' => self::$version, 'oauth_nonce' => self::generate_nonce(), 'oauth_timestamp' => self::generate_timestamp(), 'oauth_consumer_key' => $consumer->key];
...
abstract class OAuthDataStore
{
  ...
  abstract public function new_request_token($consumer);

  // return a new access token attached to this consumer
  // for the user associated with this token if the request token
  // is authorized
  // should also invalidate the request token
  abstract public function new_access_token($token, $consumer);

    abstract public function authorize_request_token($token);
}
