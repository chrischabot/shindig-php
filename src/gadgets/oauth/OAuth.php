...
class OAuthSignatureMethod_HMAC_SHA1 extends OAuthSignatureMethod
{
    ...
    public function build_signature($request, $consumer, $token)
    {
        ...
        $key_parts = [$consumer->secret, (isset($token)) ? $token : ''];
...
class OAuthSignatureMethod_PLAINTEXT extends OAuthSignatureMethod
{
  ...
  public function check_signature(&$request, $consumer, $token, $signature)
  {
      ...
      $sig = explode('&', $raw);
      array_pop($sig);
      $secret = [OAuthUtil::urldecodeRFC3986($consumer->secret)];
...
class OAuthSignatureMethod_RSA_SHA1 extends OAuthSignatureMethod
{
    ...
    $cert = $consumer->getProperty(self::$PRIVATE_KEY);
...
class OAuthRequest
{
  ...
  public static function from_request($http_method = null, $http_url = null, $parameters = null)
  {
    ...
    if ($parameters) {
      ...
      $header_parameters = self::split_header($request_headers['Authorization']);
        if ($http_method == 'GET') {
            $req_parameters = $_GET;
        ...
        $req = new self($http_method, $http_url, $_GET);
    } elseif ($http_method == 'POST') {
        $req = new self($http_method, $http_url, $_POST);
    }
  ...
  public static function from_consumer_and_token($consumer, $token, $http_method, $http_url, $parameters = null)
  {
      ...
      $defaults = ['oauth_nonce' => self::generate_nonce(),
  ...
  public function get_signable_parameters()
  {
    ...
    $keys = array_map(['OAuthUtil', 'urlencodeRFC3986'], array_keys($params));
      $values = array_map(['OAuthUtil', 'urlencodeRFC3986'], array_values($params));
  ...
  public function get_signature_base_string()
  {
      ...
      $parts = [$this->get_normalized_http_method(), $this->get_normalized_http_url(),
...
class OAuthUtil
{
    ...
    public static function getPostBodyString(array $params)
    {
        ...
        foreach ($params as $key => $val) {
            ...
            $result .= self::urlencodeRFC3986($key).'='.self::urlencodeRFC3986($val);
        }
    ...
    public static function decodeForm($form)
    {
        ...
        foreach ($explodedForm as $params) {
            ...
            if (!empty($value[0]) && !empty($value[1])) {
                $parameters[self::urldecodeRFC3986($value[0])] = self::urldecodeRFC3986($value[1]);
            }
        }
  ...
  public static function decodeAuthorization($authorization)
  {
      ...
      if ($authorization != null) {
          ...
          if ($m !== false) {
              if (strpos($authorization, self::$AUTH_SCHEME) == 0) {
                  $authorization = str_replace('OAuth ', '', $authorization);
                  $authParams = explode(', ', $authorization);
                  foreach ($authParams as $params) {
                      $m = ereg(self::$NVP, $params);
                      if ($m == 1) {
                          $keyValue = explode('=', $params);
                          $name = self::urlencodeRFC3986($keyValue[0]);
                          $value = self::urlencodeRFC3986(str_replace('"', '', $keyValue[1]));
                          $into[$name] = $value;
                      }
                  }
              }
