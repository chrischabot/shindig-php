...
class OAuthFetcher extends RemoteContentFetcher
{
  ...
  protected function lookupOAuthMetadata()
  {
    ...
    $accessor = $this->accessorInfo->getAccessor();
      ...
      } elseif (isset($this->origClientState[self::$ACCESS_TOKEN_KEY])) {
      ...
      } elseif ($accessor->accessToken == null && $this->requestParams->getRequestToken() != null) {
  ...
  {
      try {
          $msgParams = OAuthUtil::isFormEncoded($this->realRequest->getContentType()) ? OAuthUtil::urldecodeRFC3986($this->realRequest->getPostBody()) : [];
