...
class JsLibrary
{
  ...
  private $loaded = false;
        ...
        if (!$this->loaded && $this->type == 'FILE') {
            if (Config::get('compress_javascript')) {
                ...
                if (!($content = $featureCache->get(md5($this->content)))) {
                    $content = JsMin::minify(self::loadData($this->content, $this->type));
                ...
                $this->content = self::loadData($this->content, $this->type);
            }
            $this->loaded = true;
        }
    ...
    public static function create($type, $content, $name)
    {
        return new self($type, $content, $name);
    }
